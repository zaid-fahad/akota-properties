  ...
  return (
    <section className="py-24 bg-slate-100 relative" id="availability">
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="max-w-4xl mx-auto bg-white p-4 rounded-2xl shadow-sm border border-slate-200 mb-8 flex flex-wrap justify-center items-center gap-6 text-xs font-bold text-slate-800">
        ...
        <div className="max-w-4xl mx-auto bg-white rounded-2xl p-6 md:p-10 shadow-xl border border-slate-200">
          <div className="space-y-4">
            {UNIT_AVAILABILITY_MATRIX.map((row, idx) => (
              <div 
                key={idx}
                className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl bg-slate-50 border border-slate-200 hover:border-emerald-300 transition-all gap-4"
              >
                  ...
                  {row.units.map((u, uIdx) => (
                    <button
                      ...
                      className={`p-3 rounded-xl border text-left transition-all relative overflow-hidden flex flex-col justify-between ${
                        u.status === 'available'
                          ? 'bg-emerald-50 border-emerald-300 text-slate-900 hover:bg-emerald-600 hover:text-white group cursor-pointer shadow-sm'
                          : u.status === 'booked'
                    ...
                    >
                      ...
                      <span className="text-[11px] font-semibold opacity-90">{u.size}</span>
                    </button>
                  ))}
                </div>
              </div>
          ...
          <div className="mt-8 pt-6 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <button
              ...
              className="bg-emerald-600 hover:bg-emerald-500 text-white font-extrabold px-6 py-3 rounded-xl text-xs uppercase tracking-wider shadow-lg shadow-emerald-600/30 flex items-center gap-2"
            >
